  ...
  return (
    <Modal opened={clicked} onClose={close} title="Event Details" centered size="md">
      <Stack>
        ...
        {eventStart && eventEnd && onahType && (
          <Stack gap="xs">
            ...
            <Group justify="space-between">
              ...
              <Text size="sm">
                {onahIcon} {onahType === 'day' ? 'Day Onah' : 'Night Onah'}
              </Text>
            </Group>
            <Group justify="space-between">
              <Text size="sm" fw={500}>
        ...
        <Button onClick={handleUpdateNotes} loading={isSaving} disabled={isDeleting}>
          Save Notes
        </Button>

        {/* Only show delete button for period start events */}
        {isPeriodStart && (
          <Button color="red" onClick={handleDeleteCycle} loading={isDeleting} disabled={isSaving}>
            Delete Entire Cycle
          </Button>
        )}

        <Button variant="outline" onClick={close} disabled={isDeleting || isSaving}>
          Cancel
        </Button>
      </Stack>
    </Modal>
